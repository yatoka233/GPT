...
class NameDataset(Dataset):
    def __init__(self, pretraining_dataset, data):
        ...
        self.data = list(data.encode('utf-8').decode('ascii', errors='ignore').split('\n'))     # 保证只有ascii能解码的英文字符串、数字和字符，并且按行分割
    ...
    def __getitem__(self, idx):
        inp, oup = self.data[idx].split('\t')   # tab 分割
        print(inp)
        x = inp + self.MASK_CHAR + oup + self.MASK_CHAR
        x = x + self.PAD_CHAR*(self.block_size - len(x))    # pad 到 block_size
...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_len = min(int(random.gauss(1/4, 0.05)*len(doc)), len(doc)-2)
        start_idx = random.randint(1, len(doc)-mask_len-1)
