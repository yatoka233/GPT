...
if __name__ == '__main__':
    ...
    if args.function == 'pretrain':
        ...
        Mytrainer = trainer.Trainer(model, pretrain_dataset, None, train_config)
        Mytrainer.train()
        ...
        if args.reading_params_path is None:
            train_config = trainer.TrainerConfig(max_epochs=75, batch_size=256, learning_rate=6e-4, lr_decay=True, warmup_tokens=512*20,
                                                final_tokens=200*len(pretrain_dataset)*block_size, ckpt_path=args.writing_params_path, num_workers=4)
        else:
            train_config = trainer.TrainerConfig(max_epochs=10, batch_size=256, learning_rate=6e-4, lr_decay=True, warmup_tokens=512*20,
                                                final_tokens=200*len(pretrain_dataset)*block_size, ckpt_path=args.writing_params_path, num_workers=4)
            model.load_state_dict(torch.load(args.reading_params_path))

        train_dataset = dataset.NameDataset(pretrain_dataset, open(args.finetune_corpus_path).read())

        Mytrainer = trainer.Trainer(model, train_dataset, None, train_config)
        Mytrainer.train()
